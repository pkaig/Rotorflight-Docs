...
**1.** Open the [Modes tab](../configurator/tabs/modes.md)  
**2.** Set the channel to match your switch (in this case AUX1)  
**3.** Drag the sliders to the top end of the channel  
**4.** Save  
**5.** Toggle the channel and watch the red indicator pip move. When it is inside the upper range the ARM box turns red.  
...
The swashplate is configured to indicated ready for Arming as well as Errors. More details can be found on the [Swashplate wiggle](../setup/swash-wiggle.mdx) page.
...
|   'THROTTLE'        | Lower throttle below [Throttle Channel value for arming](../configurator/tabs/receiver.mdx#receiver-settings)                 |
...
|   'LOAD'            | System load is too high. This may happen with some low spec boards. Reduce the [PID loop frequency](../configurator/tabs/configuration.md#system-configuration) and/or number of filters|
...
|   'RPMFILTER'       | RPM-based filtering is enabled but one or more ESC's are not configured or receiving valid RPM signals. RPM feedback can be from bi-directional Dshot or Frequency inputs using the ESC RPM signal or an external RPM sensor. See [RPM Measurement](../setup/rpm-measurement.mdx) for more details.|
