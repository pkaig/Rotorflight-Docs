...
<Tabs>
    <TabItem value="Purpose" label="Purpose" default>
        There are several aerodynamic effects that result in the helicopter rolling slightly while pitching and pitching slightly when rolling. Cyclic cross coupling provides a small correction to the opposite axis so that the "cross coupling" is minimised and the helicopter only moves on the commanded axis. When this parameter is tuned well the PID control loop does not have to work very hard to keep the helicopter on its commanded axis.    
    ...
    <TabItem value="Intermediate" label="Intermediate">
    1. From a hover. Pitch the helicopter forward and backwards as rapidly as possible.  
...
<Tabs>
    ...
    <TabItem value="Beginner" label="Beginner">
        ...
        2. When vibrations are seen, reduce your P value 
    <div style={{textAlign: 'center', fontWeight: "bold"}}>
        Video to go here
    </div>
    </TabItem>
    <TabItem value="Intermediate" label="Intermediate">
    1. Increase P until the helicopter begins to 
    <div style={{textAlign: 'center', fontWeight: "bold"}}>
        Video to go here
    </div>
    </TabItem>
