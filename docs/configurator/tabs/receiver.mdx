...
* It is preferable that a telemetry capable protocol be used with the [Rotorflight LUA scripts](/docs/setup/Lua-Scripts.mdx) to perform tuning using you radio.  
...
The stability modes (Angle, Horizon or Rescue) can be calibrated so that the heli doesn't drift much when using such a mode. This can only be done via the [accelerometer trims](/docs/setup/Using-Stability-Modes-example.md). 
...
:::tip
For ELRS receivers, follow the [ELRS Custom Telemetry](/docs/setup/elrs-custom-telemetry.mdx) page for how to setup. This is new for Rotorflight 2.1 and ELRS Re-use is no longer available.  
:::
