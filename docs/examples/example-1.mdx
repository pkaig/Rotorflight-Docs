...
## Update Firmware
Update the Flight Controller firmware to the current release. Follow the description on the [**Flashing the firmware**](/docs/setup/Flashing-the-firmware.mdx) page. 
...
Open the [configuration tab](/docs/configurator/tabs/configuration.md) and set the serial port **Port (A)** to **Serial_RX**.  
**Save and Reboot**.

![Example 1](./img/example-1-3.png)

### Receiver -  Rx protocol
Open the [Receiver tab](/docs/configurator/tabs/receiver.mdx). Set the receiver protocol to TBS CRSF (ELRS) and the channel order to ELRS (AECR1T).  
**Save and Reboot**.
...
## Enable ARM mode
Arming is an important step for Rotorflight. See the [**Arming setup**](/docs/setup/arming.mdx) page for more details.  
...
## Servo Setup
The purpose of this step is to [**setup and calibrate servos**](/docs/setup/setup-servos.mdx). On the M4 the servos are setup in the [CCPM 120 degree](/docs/configurator/tabs/servos.mdx#ccpm) arrangement so the servos need to be plugged in as shown below.    

![Example 1](../configurator/img/ccpm.svg)  

## Mixer Setup
The purpose of this step is to [**setup and calibrate the mixer**](/docs/setup/setup-mixer.mdx).  
...
**RPM Sensor** - The nexus has an RPM input. The Yellow RPM signal wire from the ESC is plugged into this port. Enabling this toggle enables the RPM so it can be used for the [**Governor**](/docs/setup/Governor.md) and [**RPM Filtering**](/docs/setup/RPM-Filters.md)  
...
See the [**Govenor Modes**](/docs/setup/Governor.md#governor-mode) and [**Governor setup**](/docs/setup/governor-setup.mdx) for details and examples.
...
See the [**ELRS Custom Telemetry**](/docs/setup/elrs-custom-telemetry.mdx) on how to set it up.
